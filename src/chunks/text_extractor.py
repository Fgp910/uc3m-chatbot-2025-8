...
class VisionOCR:
    """Handle Vision API calls with retry logic."""
    ...
    def extract_page(
        ...
        """
        Extract text from a page using Vision API.
        
        Returns:
            (text, retry_count, errors) or (None, retry_count, errors) if failed
        """
        errors = []
                ...
                response = self.client.messages.create(
                    ...
                    messages=[{
                        ...
                        "content": [
                            ...
                            {
                                "type": "text",
                                "text": """Extract ALL text from this document page exactly as written.

CRITICAL INSTRUCTIONS:
- Extract every word, number, dollar amount, and date
- For tables: preserve structure using | as column separator
- Include headers, footers, page numbers
- If text is unclear, write [unclear] but still attempt extraction
...
Output ONLY the extracted text."""
                ...
                self.total_calls += 1
                self.total_cost += COST_PER_VISION_CALL
                
                text = response.content[0].text
                
                # Basic validation of Vision response
                if len(text.strip()) < MIN_VISION_RESPONSE:
                    errors.append(f"Vision returned only {len(text)} chars")
                    if attempt < MAX_RETRIES - 1:
                        time.sleep(RETRY_DELAYS[attempt])
                        continue
                
                return text, attempt, errors
                
            except anthropic.APIError as e:
                error_msg = f"API error (attempt {attempt + 1}): {str(e)[:100]}"
...
class ProductionTextExtractor:
    ...
    def __init__(
        ...
        Args:
            api_key: Anthropic API key for Vision
            checkpoint_dir: Directory for checkpoints
            enable_checkpoint: Whether to use checkpointing
        """
        self.vision = VisionOCR(api_key)
    ...
    def extract_document(self, pdf_path: Path) -> DocumentResult:
        ...
        for page_num in range(num_pages):
            ...
            # Need Vision OCR
            vision_text, retry_count, vision_errors = self.vision.extract_page(
                pdf_path, page_num
            )
            vision_cost += COST_PER_VISION_CALL
            
            if vision_text:
                text_cleaned = clean_text(vision_text)
                success, confidence, page_issues = validate_text_quality(
                    vision_text, page_num, 'vision'
                )
                page_issues.extend(vision_errors)
                ...
                pages.append(PageResult(
                    ...
                    success=success,
                    confidence=confidence,
                    printable_ratio=calculate_printable_ratio(vision_text),
                    issues=page_issues,
                ...
                # Vision failed - use whatever PyMuPDF got
                text_cleaned = clean_text(raw_text)
                ...
                pages.append(PageResult(
                    ...
                    success=False,
                    confidence=0.2,  # Low confidence for failed Vision
                    printable_ratio=printable_ratio,
                    issues=["Vision API failed"] + vision_errors,
                    retry_count=retry_count
                ))
